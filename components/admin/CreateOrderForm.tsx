...
function emptyRow(): LineItemRow {
  return { rowId: crypto.randomUUID(), productId: '', variantId: '', quantity: '1', unitPrice: 0 }
}
...
export default function CreateOrderForm() {
  ...
  // Keep totalInput in sync with computed total unless admin has manually overridden it
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const line_items = validItems.map((row) => {
      ...
      return {
        ...
        variant_label: getVariantLabel(variant) || null,
